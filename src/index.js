...
class GH_Card extends HTMLElement {
  ...
  set_component_config() {
    ...
    // SET BASE COLOR
    this.component_config["colors"]["primary"] =
      this.getAttribute("gh-primary-color");
    this.component_config["colors"]["secondary"] =
      this.getAttribute("gh-secondary-color");
  ...
  set_template_styles() {
    ...
    this.template_styles["normal"] = basic_style(
      this.component_config["colors"]
    );
    this.template_styles["horizontal"] = horizontal_style(
      this.component_config["colors"]
    );
    this.template_styles["horizontal-projects"] = horizontal_projects_styles(
      this.component_config["colors"]
    );
  ...
  fetchData() {
    ...
    let github_profile_url = `https://api.github.com/users/${this.getAttribute(
      "gh-user"
    )}`;
    let fallowers_url = `https://api.github.com/users/${this.getAttribute(
      "gh-user"
    )}/followers`;
    let fallowing_url = `https://api.github.com/users/${this.getAttribute(
      "gh-user"
    )}/following`;
    let repos_url = `https://api.github.com/users/${this.getAttribute(
      "gh-user"
    )}/repos`;
  ...
  build_card() {
    ...
    if (this.getAttribute("gh-mode") == "horizontal-projects") {
      ...
      for (let i of [...Array(3).keys()]) {
        ...
        repos_container.appendChild(
          repo_layout(this.userData["repos"][index_repo])
        );
      }
    }
  }
